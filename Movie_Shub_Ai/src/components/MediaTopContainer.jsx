...
import { GET_API_OPTION, MOVIE_DATABASE_API, SERIES_DATABASE_API, TMDB_IMAGE_ORIGIN } from "../utils/Constant";
  ...
  async function getYoutubeKey() { 
    if (mediaType === "movie") {
      const res = await fetch(
        MOVIE_DATABASE_API + id +"/videos?language=en-US",
        GET_API_OPTION
      );
      ...
      const res = await fetch(
        SERIES_DATABASE_API + id + "/videos?language=en-US",
        GET_API_OPTION
      );
      const keyData = await res.json();
