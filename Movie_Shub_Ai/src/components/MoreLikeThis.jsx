...
function MoreLikeThis({ id, mediaType, setIsMoreLikeThis }) {
  ...
  async function getSimilarMovieOrTvData() {
    if (mediaType == "movie") {
      const response = await fetch(
        MOVIE_DATABASE_API + id + "/similar?language=en-US&page=1",
        GET_API_OPTION
      );
      ...
      const response = await fetch(
        SERIES_DATABASE_API + id + "/similar?language=en-US&page=1",
        GET_API_OPTION
      );
      const resData = await response.json();
  ...
  async function getRecommendedMovie() {
    const response = await fetch(
      MOVIE_DATABASE_API + id + "/recommendations?language=en-US&page=1",
      GET_API_OPTION
    );
    const resData = await response.json();
    setData(resData?.results);
  ...
  async function getRecommendedSeries() {
    const response = await fetch(
      SERIES_DATABASE_API + id + "/recommendations?language=en-US&page=1",
      GET_API_OPTION
    );
    const resData = await response.json();
    setData(resData?.results);
      ...
      <div className="xl:w-[75%] lg:w-[80%] md:w-[90%] w-[96%] mx-auto">
        <div className="w-full">
          <div className="flex gap-1 items-center">
            ...
            <span className="lg:text-[25px] md:text-text-[22px] sm:text-xl text-lg font-semibold">
              {header}
            </span>
          </div>
